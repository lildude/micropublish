...
module Micropublish
  class Compare
    ...
    def sanitize
      ...
      @existing.each { |k, _v| @existing.delete(k) unless @known.include?(k) }

      # remove properties starting with _ or mp- (used internally)
      @submitted.each { |k, _v| @submitted.delete(k) if k.start_with?('_', 'mp-') }
    end
